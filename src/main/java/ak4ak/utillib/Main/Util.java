...
public class Util {
    ...
    public static Component colorize(String text) {
        ...
        // 0. Сброс стилей + белый цвет по умолчанию
        String processed = "<reset><white>" + text;
    ...
    public static void command(Player player, String command){
        ...
        if (!event.isCancelled()) {
            Bukkit.dispatchCommand(player, event.getMessage().substring(1));
        }
...
//Ищет любую цель которвя находится на экране игрока, сначала идет те цели которые ближе всего к экрану
    ...
    public static LivingEntity getTargetEntityAim(Player player, double radius) {
        ...
        public static void minusDurability(ItemStack itemStack, int durability, Player player){
            ...
            int newDamage = currentDamage + durability;
            ...
            if (customStack.getDurability() > 1) {
